    ...
    methods: {
        login() {
            ...
            &password=${this.userPassword}`,
                ...
                .then(() => window.location.href = '/webb/pages/accounts.html')
            ...
            axios.post('/api/logout')
                .then(() => Swal.fire({
                    ...
                    showConfirmButton: false,
                    timer: 1500
                })).then(() => window.location.href = '/webb/pages/login.html/')



        }, register() {
            axios.post('/api/clients', `name=${this.nameValue}&lastName=${this.lastNameValue}&email=${this.newEmail}&password=${this.newPassword}`, { headers: { 'content-type': 'application/x-www-form-urlencoded' } })
                .then(() => axios.post('/api/login',`email=${this.newEmail}&password=${this.newPassword}`,
                    { headers: { 'content-type': 'application/x-www-form-urlencoded' } }))
                .then(() =>
                    axios.post('/api/clients/current/accounts', `accountType=SAVING`, { headers: { 'content-type': 'application/x-www-form-urlencoded' } }))
                .then(() => Swal.fire({
                    position: 'center',
                    icon: 'success',
                    title: 'Your Account has been successfully',
                    showConfirmButton: false,
                    timer: 1500
                }))
                .then(() => window.location.href = '/webb/pages/accounts.html')

        ...
        loadClientList() {
            axios.get('/api/clients/current')
                .then(res => {
                    ...
                    this.loans = this.clients.loans.sort((a, b) => a.id - b.id);
                    this.cards = this.clients.cards.sort((a, b) => a.id - b.id);
                    this.cardActive = this.cards.filter(a => a.cardActive)
            ...
            axios.post('/api/logout')
                .then(() =>
                    window.location.href = '/webb/pages/login.html/')
            ...
            swalWithBootstrapButtons.fire({
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        axios.patch('/api/clients/current/cards', `number=${this.cardNumber}`, { headers: { 'content-type': 'application/x-www-form-urlencoded' } })
                            .catch(error => Swal.fire({
                                icon: 'error',
                                text: `${error.response.data}`
                            }))
            ...
            swalWithBootstrapButtons.fire({
                title: 'Are you sure?',
                text: "you are about to perform a transaction!",
                icon: 'warning',
                showCancelButton: true,
                ...
                .then((result) => {
                    if (result.isConfirmed) {
                        ...
                        swalWithBootstrapButtons.fire(
                            'Transactions!',
                            'your transation was successfully',
                            'success'
                        ).then(() => window.location.reload())
                    } else if (
                        result.dismiss === Swal.DismissReason.cancel
            ...
            axios.get('/api/clients/')
                .then(res => {
                    ...
                    this.thruDate = this.cards.thruDate;
            ...
            axios.post('/api/transactions/downloads', { fromDate: `${this.fromDate}`, thruDate: `${this.toDate}`, account: `${this.accountNumber}` })
                .then(() =>
                    Swal.fire(
                        'Good job!',
                        'You clicked the button!',
